...
## Ещё немного Selenide API — клик, `<select>`, коллекции (для P3-T2)

```java
element.click(); // клик, сам ждёт кликабельности перед действием

element.selectOption("Опция 1");           // выбор в обычном HTML <select> по видимому тексту
String selected = element.getSelectedOptionText(); // текущий выбранный текст

ElementsCollection rows = element.$$x(".//tr"); // коллекция элементов ВНУТРИ уже найденного element (не с нуля со страницы); $$x — для XPath, $$ — для CSS
rows.size();       // сколько элементов нашлось
rows.get(0);        // SelenideElement конкретной строки
rows.texts();       // List<String> — текст каждого элемента коллекции разом
```

`$$` — то же самое, что `$`, но возвращает НЕСКОЛЬКО элементов (`ElementsCollection`) вместо одного. Вызванный на уже найденном `element` (а не на `Selenide.$$` статически), он ищет ВНУТРИ этого элемента — то есть у обёртки таблицы `element.$$(".//tr")` найдёт строки именно этой таблицы, а не все `<tr>` на странице.
