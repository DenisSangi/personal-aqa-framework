...
- [P10-T2 — Поиск + корзина: 2–3 сценария](P10-T2-search-cart-checkout.md) — ⚠️ Conditional Accepted 2026-08-03: вся логика на месте, но автономного зелёного прогона нет (рекламный оверлей перехватывает клики) — стабилизация вынесена в P10-T3.
- [P10-T3 — Стабилизация UI-тестов](P10-T3-ui-stabilization.md) — ⚠️ закрыта 2026-08-04 как **задокументированное ограничение**: причина разобрана до конца (рекламный скрипт перехватывает само событие клика, а не перекрывает элемент визуально), но ни один из четырёх подходов её не устранил. Побочный результат — рабочий `IRetryAnalyzer` + `IAnnotationTransformer` в проекте. Отложено: сузить retry до `TimeoutException`/`StaleElementReferenceException`.
